...
/** Financial table column keys (18) — visibility + highlight */
type StatsColKey =
  | "vendor"
  | "security"
  | "manualPayIn"
...
function defaultColBoolMap(on: boolean): Record<StatsColKey, boolean> {
  return Object.fromEntries(FIN_STATS_COLUMNS.map((c) => [c.key, on])) as Record<StatsColKey, boolean>;
}

function isInactiveVendorRow(r: VendorRow): boolean {
  return (
    r.running === 0 &&
    r.runningUnsettled === 0 &&
    r.netPayIn === 0 &&
    r.payout === 0 &&
    r.credit === 0 &&
    r.security === 0 &&
                  ...
                  <td className={`${colCell} ${colActions}${xh("actions")}${xv("actions")}`}>
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          className={`pointer-events-none absolute right-5 top-1/2 -translate-y-1/2 z-20 w-fit whitespace-nowrap rounded-md border border-gray-200 dark:border-gray-700 bg-white/95 dark:bg-gray-900/95 px-2 py-1 text-[11px] font-medium text-gray-600 dark:text-gray-200 shadow-sm text-right transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                            rowHover?.slot === 0 ? "-translate-x-2 opacity-100" : "translate-x-2 opacity-0"
                          }`}
                        >
                          View
                        </span>
                      ...
                      >
                        ...
                        <span
                          className={`pointer-events-none absolute right-5 top-1/2 -translate-y-1/2 z-20 w-fit whitespace-nowrap rounded-md border border-gray-200 dark:border-gray-700 bg-white/95 dark:bg-gray-900/95 px-2 py-1 text-[11px] font-medium text-gray-600 dark:text-gray-200 shadow-sm text-right transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                            rowHover?.slot === 1 ? "-translate-x-2 opacity-100" : "translate-x-2 opacity-0"
                          }`}
                        >
                          Remove
                        </span>
