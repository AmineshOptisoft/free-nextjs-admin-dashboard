...
export async function POST(req: Request) {
  ...
  if (!clientName || !clientUpi || !bankName || !accountNo || !ifsc) {
    return NextResponse.json({ ok: false, error: "Client/bank fields required" }, { status: 400 });
  }
  ...
  const [res] = await pool.execute<ResultSetHeader>(
    ...
    ) VALUES (?, 'PAYOUT', ?, ?, 'INR', 'BANK', ?, ?, ?, ?, 'NOT_ASSIGNED', ?, ?, ?, NULLIF(?, ''))`,
    [randomCode, orderId, amount, accountNo, ifsc, bankName, clientName, clientName, clientUpi, auth.companyId, note],
  );
  ...
  const row = rows[0];
  if (!row) return NextResponse.json({ ok: false, error: "Could not load created payout" }, { status: 500 });
