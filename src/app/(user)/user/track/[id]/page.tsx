...
export default function OrderTrackingPage() {
  ...
  const fetchOrder = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) router.push("/user");
        return;
      }
  ...
  const handleCancel = async () => {
    ...
    setCancelling(true);
    try {
      const res = await fetch(`/api/ride-requests/${id}/cancel`, { method: "POST" });
      if (res.ok) {
         fetchOrder(); // Reload the UI to show Cancelled state
      }
    } catch(e) {
      console.error(e);
    } finally {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 relative flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-[450px] bg-white dark:bg-gray-900 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-gray-800 shadow-2xl z-[1002] flex flex-col justify-between">
          <div className="p-8 space-y-8 overflow-y-auto">
            ...
            {order?.status === "Cancelled" ? (
              <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-6 rounded-2xl text-center border border-red-200 dark:border-red-800 space-y-4">
                ...
                <div>
                  <h3 className="text-lg font-black uppercase tracking-widest leading-tight">Mission Cancelled</h3>
                  <p className="text-sm font-medium opacity-80 mt-1">This ride request has been aborted.</p>
                </div>
                <button 
                  onClick={() => router.push('/user/book-ride')}
                  className="w-full py-3 bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-widest rounded-xl transition-all shadow-md text-xs"
                >
                  Book New Ride 🚀
                </button>
              </div>
          ...
          <div className="p-6 bg-gray-50 dark:bg-white/5 border-t border-gray-200 dark:border-gray-800 space-y-3">
            ...
            {order?.status !== "Cancelled" && order?.status !== "Delivered" && order?.status !== "Started" && (
              <button 
                ...
                disabled={cancelling}
                className="w-full h-12 border-2 border-red-500/20 text-red-500 hover:bg-red-500/10 font-black uppercase tracking-widest rounded-xl transition-all text-[10px] disabled:opacity-50"
              >
