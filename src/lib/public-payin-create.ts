...
import { createSpeedpayPayin } from "@/lib/integrations/speedpay/controller";
...
export type PublicPayInCompany = {
  id: number;
  status: string;
};
...
export type PublicPayInCreateResult =
  | { ok: true; status: 200; body: { ok: true; request: Record<string, unknown>; waitForAgent: boolean } }
...
function speedpayPayinEnabled(): boolean {
  return process.env.SPEEDPAY_PAYIN_ENABLED?.trim() === "1";
}
...
export async function createPublicPayIn(
  company: PublicPayInCompany,
  body: Record<string, unknown>,
): Promise<PublicPayInCreateResult> {
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    return { ok: false, status: 400, body: { ok: false, error: "Valid amount required" } };
  }
  if (!clientName) {
    return { ok: false, status: 400, body: { ok: false, error: "Client name is required" } };
  }
  if (!preferredMethod) {
    return { ok: false, status: 400, body: { ok: false, error: "Method must be UPI or BANK" } };
  }

  if (speedpayPayinEnabled()) {
    try {
      const created = await createSpeedpayPayin({
        amount,
        note: `company:${company.id}|${clientName}`,
      });
      return {
        ok: true,
        status: 200,
        body: {
          ok: true,
          request: {
            id: `sp_${created.providerId}`,
            orderId: created.transactionNumber,
            amount: created.amount,
            status: created.providerStatus,
            displayStatus: created.displayStatus,
            clientName,
            clientUpi: clientUpi || "",
            assignedUpi: created.upi,
            bankName: "",
            accountNo: "",
            ifsc: "",
            paymentMethod: "UPI",
            ...
            hasReceipt: false,
            utrCode: created.referenceNumber ?? "",
            qrCodeUrl: created.redirectUrl,
          },
          waitForAgent: false,
        },
      };
    } catch (e) {
      const message = e instanceof Error ? e.message : "Could not create payment request";
      return { ok: false, status: 502, body: { ok: false, error: message } };
    }
  }
  ...
  async function loadTxById(id: number): Promise<TxRow | null> {
    const [rows] = await pool.execute<TxRow[]>(
      `SELECT \`id\`, \`order_id\`, \`amount\`, \`status\`, \`client_name\`, \`client_upi\`, \`assigned_upi\`,
              ...
              \`pay_method_id\`, \`assigned_agent_id\`, \`utr_code\`, \`payment_image\`, \`qr_code_url\`, \`user_note\`, \`expires_at\`
  ...
  function respondWithTx(tx: TxRow): PublicPayInCreateResult {
    const mapped = mapRequest(tx);
    void sendPayinWebhookForTx(Number(tx.id), { event: "payin.initiated" });
    return { ok: true, status: 200, body: { ok: true, request: mapped, waitForAgent: mapped.waitForAgent } };
  }

  function noEligiblePayInResponse(detail?: string): PublicPayInCreateResult {
    const msg = detail?.trim() ? detail.trim() : PAYIN_NO_ELIGIBLE_AGENT_MESSAGE;
    return { ok: false, status: 503, body: { ok: false, error: msg } };
  }

  async function assignPayInOr503(txId: number, amt: number, method: "UPI" | "BANK" | ""): Promise<PublicPayInCreateResult | null> {
    try {
  ...
  if (idempotencyKey) {
    ...
    if (ex) {
      ...
      if (!fresh) return { ok: false, status: 500, body: { ok: false, error: "Could not load request" } };
  ...
  try {
    const [res] = await pool.execute<ResultSetHeader>(
      ...
      [randomCode, orderId, amount, preferredMethod, clientName, clientUpi, company.id, idemParam, userNote || null],
    );
  ...
  if (!tx) return { ok: false, status: 500, body: { ok: false, error: "Could not load created request" } };
